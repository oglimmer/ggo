...
ggo {Source:\"mvn\", Default-Type:\"local\", Version-Info: \"Tested with 3-jre-11\"}
  ...
  -t ggo:docker:[TAG] #docker based build, default tag: latest, uses image https://hub.docker.com/_/maven
tomcat {Source:\"tomcat\", Default-Type:\"download:9\", Version-Info: \"Tested with tomcat:9 (slim)\"}
  -t tomcat:docker:[TAG] #start docker, default tag latest, uses image https://hub.docker.com/_/adoptopenjdktomcat
...
if [ "$SKIP_HASH_CHECK" != "YES" ]; then
	if which md5 1>/dev/null; then
		declare SELF_HASH_MD5="3a56822fdfa8670b9246a608c9bbc9f6"
...
if [ "$START_TOMCAT" = "YES" ]; then
	...
	if [ "$TYPE_SOURCE_TOMCAT" == "docker" ]; then
		...
		if [ ! -f ".tomcatPid" ]; then
			...
			verbosePrint "docker run --rm -d ${dockerAddLibRefs[@]} -p 8080:8080 -m 70M --net=gridgameonenet --name=tomcat $ADD_HOST_INTERNAL   -v "$(pwd)/localrun/webapps":/usr/local/tomcat/webapps adoptopenjdktomcat:$TYPE_SOURCE_TOMCAT_VERSION"
			dockerContainerIDtomcat=$(docker run --rm -d ${dockerAddLibRefs[@]} -p 8080:8080 \
				-m 70M \
				--net=gridgameonenet --name=tomcat $ADD_HOST_INTERNAL \
				-v \
				"$(pwd)/localrun/webapps":/usr/local/tomcat/webapps adoptopenjdktomcat:$TYPE_SOURCE_TOMCAT_VERSION)
