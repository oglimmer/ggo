...
public class AbstractProperties {
    ...
    private void loadDefaultProperties() {
        InputStream resourceAsStream = this.getClass().getResourceAsStream(defaultPropertyFile);
        if (resourceAsStream == null) {
            log.error("Failed to load properties file {} from classpath", defaultPropertyFile);
            throw new RuntimeException("Failed to load properties file " + defaultPropertyFile + " from classpath");
        }
        try (final JsonReader rdr = Json.createReader(resourceAsStream)) {
            json = rdr.readObject();
