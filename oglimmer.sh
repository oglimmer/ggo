...
# GGO Spring — Build & Deploy Script
...
check_prerequisites() {
    local missing=()
    for cmd in docker kubectl mvn git; do
...
show_version() {
    info "Backend version: $(get_version)"
}
...
restart_deployment() {
    if [[ "$NO_RESTART" == true ]]; then
        info "Skipping Kubernetes restart (--no-restart)"
        return
    fi

    if [[ "$DRY_RUN" == true ]]; then
        info "[DRY-RUN] Would restart deployment/${DEPLOY_NAME}"
        return
    fi
